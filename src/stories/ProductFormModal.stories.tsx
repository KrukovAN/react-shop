...
} from "@/modals/product-form-modal";
...
function ProductFormModalDemo({ mode, initialValue }: ProductFormStoryArgs) {
  ...
  const handleValidation = (result: ProductFormValidationResult) => {
    console.log("[ProductFormModal] validation", result);
  };
    ...
    <div className="w-[min(100%,920px)] space-y-4 rounded-3xl border bg-card p-6 shadow-sm">
      <div className="flex flex-wrap items-center gap-2">
        <Button type="button" onClick={openAdd}>
          РћС‚РєСЂС‹С‚СЊ РґРѕР±Р°РІР»РµРЅРёРµ
        </Button>
        <Button type="button" variant="secondary" onClick={openEdit}>
          РћС‚РєСЂС‹С‚СЊ СЂРµРґР°РєС‚РёСЂРѕРІР°РЅРёРµ
        </Button>
      </div>
...
const meta: Meta<ProductFormStoryArgs> = {
  title: "Р¤РѕСЂРјС‹/Р¤РѕСЂРјР° С‚РѕРІР°СЂР°",
  parameters: {
    layout: "centered",
  },
  args: {
    ...
    initialValue: {
      id: "prd_demo_001",
      name: "РљРѕС„РµРјР°С€РёРЅР°",
      description: "РђРІС‚РѕРјР°С‚РёС‡РµСЃРєР°СЏ РєРѕС„РµРјР°С€РёРЅР° СЃ СЂСѓС‡РЅС‹Рј РєР°РїСѓС‡РёРЅР°С‚РѕСЂРѕРј.",
      categoryName: "РўРµС…РЅРёРєР°",
      price: 24999.9,
      oldPrice: 31999.9,
